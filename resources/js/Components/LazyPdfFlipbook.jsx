...
const PdfFlipbook = () => {
    ...
    // Ambil PDF dari IndexedDB
    const getPdfUrl = () => {
        return new Promise((resolve) => {
            const openReq = indexedDB.open("pdfDatabase", 1);
            ...
            openReq.onsuccess = (e) => {
                ...
                const getReq = store.get(props.file_pdf);

                getReq.onsuccess = (e) => {
                    const blob = e.target.result;
                    if (blob) {
                        // ✅ PDF ditemukan di IndexedDB
                        const url = URL.createObjectURL(blob);
                        resolve(url);
                    } else {
                        // ❌ Tidak ada di IndexedDB → fallback ke server
                        resolve(`/pdf/${props.file_pdf}`);
                    }
                };

                getReq.onerror = () => resolve(`/pdf/${props.file_pdf}`);
            };

            openReq.onerror = () => resolve(`/pdf/${props.file_pdf}`);
        });
    };
    ...
    const renderPdf = async () => {
        ...
        try {
            const finalPdfUrl = await getPdfUrl();

            const loadingTask = pdfjs.getDocument(finalPdfUrl);
